/**
 * Exact cross-playbook offensive art reuse (CFB27 offense only).
 *
...
export const OFFENSIVE_REUSE_GAME_VERSION = "cfb27";
export const OFFENSIVE_REUSE_SIDE: VideoSideOfBall = "offense";
...
export type OffensiveArtCorpus = Map<string, ValidatedArtEntry[]>;

export type OffensiveReuseApplyResult = {
...
export type OffensiveReuseCoverage = {
  expected: number;
...
export function offensiveReusableArtKey(
  formation: string,
  play: string,
  gameVersion = OFFENSIVE_REUSE_GAME_VERSION,
  side: VideoSideOfBall = OFFENSIVE_REUSE_SIDE,
): string {
  return `${gameVersion}\0${side}\0${formation}\0${normalizeCanonicalOffensivePlayName(play)}`;
}

export function isOffensiveReuseEligible(
  gameVersion: string,
  side: VideoSideOfBall,
): boolean {
  return gameVersion === OFFENSIVE_REUSE_GAME_VERSION && side === OFFENSIVE_REUSE_SIDE;
}
...
export function isValidatedReusableArtCard(card: ExtractedVideoCard): boolean {
  if (card.gameVersion !== OFFENSIVE_REUSE_GAME_VERSION) return false;
  if (card.side !== OFFENSIVE_REUSE_SIDE) return false;
...
export function globalRecoveredRegistryPath(playArtRoot: string): string {
  return join(
    ...
    "OFFENSIVE_GLOBAL_RECOVERY.json",
  );
}
...
export function loadDirectValidatedCards(stagingRoot: string): ExtractedVideoCard[] {
  ...
  const push = (card: ExtractedVideoCard) => {
    const key = offensiveCardIdentityKey(card);
    if (!key || seen.has(key)) return;
    if (!isDirectValidatedArtCard(card)) return;
    seen.add(key);
    cards.push(card);
  };
  ...
  const supplement = loadSupplementReport(stagingRoot);
  if (supplement) {
    for (const card of supplement.videoOnlyCards) push(card);
    for (const card of supplement.cards) {
      if (card.supplementClass === "NEW_MISSING_PLAY") push(card);
    }
    const report = loadVideoReport(stagingRoot);
    for (const card of report?.cards ?? []) push(card);
    return cards;
  }
  const report = loadVideoReport(stagingRoot);
  if (report?.cards) {
...
export function listOffensivePlaybookSlugs(playArtRoot: string): string[] {
  const root = join(
    playArtRoot,
    "video-staging",
    OFFENSIVE_REUSE_GAME_VERSION,
...
/**
 * Build corpus from direct validated captures across all offensive playbooks.
 * Sorted deterministically per reusable-art key.
 */
export function buildOffensiveValidatedArtCorpus(
  playArtRoot: string,
): OffensiveArtCorpus {
  const corpus: OffensiveArtCorpus = new Map();
  ...
  for (const slug of listOffensivePlaybookSlugs(playArtRoot)) {
    ...
    for (const card of loadDirectValidatedCards(stagingRoot)) {
      ...
      const key = offensiveReusableArtKey(formation, play);
...
export function buildRecoveredArtForBook(input: {
  ...
  const reusableArtKey = offensiveReusableArtKey(input.formation, input.play);
...
export function applyRecoveredExistingSource(input: {
  ...
  for (const formation of input.reference.formations) {
    for (const play of formation.plays) {
      const identityKey = offensivePlayIdentityKey(formation.name, play);
      if (ownedKeys.has(identityKey)) continue;

      const artKey = offensiveReusableArtKey(formation.name, play);
      const source = input.globalRecovered.find(
        (c) =>
          c.matchedFormation === formation.name &&
          offensivePlayNamesEquivalent(c.matchedPlay!, play),
      );
      if (!source) continue;
...
export function computeOffensiveReuseCoverage(input: {
  reference: PlayArtReference;
  directCards: ExtractedVideoCard[];
  combinedCards: ExtractedVideoCard[];
}): OffensiveReuseCoverage {
 ...
 */
export function applyOffensiveCrossPlaybookReuse(input: {
  targetPlaybookSlug: string;
  targetDisplayName: string;
  reference: PlayArtReference;
  directCards: ExtractedVideoCard[];
  corpus: OffensiveArtCorpus;
}): OffensiveReuseApplyResult {
  const ownedKeys = new Set<string>();
  for (const card of input.directCards) {
    const key = offensiveCardIdentityKey(card);
    if (key) ownedKeys.add(key);
  ...
  const rejected: OffensiveReuseApplyResult["rejected"] = [];
  ...
  for (const formation of input.reference.formations) {
    for (const play of formation.plays) {
      ...
      if (
        ...
        !offensivePlayNamesEquivalent(source.card.matchedPlay!, play)
      ) {
        rejected.push({
          formation: formation.name,
          play,
