...
export function CallSheetBuilderSituationHeader({
  ...
  /** When true (e.g. 25/25), emphasize the count with warning color. */
  playCountAtCapacity?: boolean;
  showEdit?: boolean;
  onEdit?: () => void;
  ...
  const playCountClassName = cn(
    "shrink-0 whitespace-nowrap font-body tabular-nums",
    playCountAtCapacity ? "font-medium text-amber-400" : "text-slate-400",
  );

  if (workspace) {
    return (
      <div className="mb-4 flex min-w-0 items-center justify-between gap-3 overflow-hidden md:mb-5 md:gap-4">
        <div className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <div className="min-w-0 flex-1 overflow-hidden">
            <h1 className="truncate font-sans text-xl font-bold uppercase leading-tight tracking-[0.06em] text-white md:text-2xl">
              {title}
            </h1>
            {subtitle ? (
              <p className="mt-1 truncate font-body text-xs text-slate-500 md:text-sm">{subtitle}</p>
            ) : null}
          </div>
        </div>

        <div className="flex shrink-0 items-center gap-2 md:gap-3">
          <p className={cn(playCountClassName, "text-xs md:text-sm")}>{playCountLabel}</p>
          {showEdit && onEdit ? (
            <button
              type="button"
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <p className={cn(playCountClassName, "pt-1 text-sm")}>{playCountLabel}</p>
      </div>
    </div>
  );
}
