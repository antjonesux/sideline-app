...
export function PlaySuggestions({
  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-900 p-4 mt-6">
      ...
      <ul className="mt-3 space-y-3">
        {suggestions.map((s) => {
          ...
          return (
            <li key={id} className="rounded-xl border border-slate-700 bg-slate-900 p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <Button
                ...
                variant="secondary"
                size="sm"
                className="shrink-0 px-3 py-2 text-xs text-emerald-300 hover:border-emerald-600/50 hover:bg-emerald-500/10 disabled:opacity-50"
                disabled={busyId === id}
                onClick={() => onAdd(s)}
              >
                {scenarioFull ? "Replace a play" : "Add play"}
              </Button>
            </li>
          );
        })}
